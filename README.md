...
Dashboard Web, Simulação Contínua, Injeção de Dados e Monitoramento via MQTT
...
```text
C213_PROJETO_2/
├── test_def.py           # Núcleo: controlador fuzzy + modelo físico + laço de simulação + MQTT
├── dashboard_server.py   # Servidor Flask + cliente MQTT (ponte HTTP ↔ MQTT)
├── index.html            # Dashboard Web (gráficos, setpoint, comandos, injeção manual)
...
4. **Abra o dashboard no navegador**
   ...
   - O arquivo `index.html` é servido pelo Flask e mostra:
...
5. **(Opcional) Usar o subscriber para debug**
   ...
   Mostra no terminal tudo que é publicado em `c213/crac/estado`.
...
- `c213/crac/estado`  
  ...
  - Carrega a telemetria completa: temperatura, erro, Δerro, potência, setpoint, Text, carga, flags de estado.  
